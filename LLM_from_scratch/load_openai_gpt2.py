...
import torch
import torch
from llm.components import GPTModel
import numpy as np
...
def load_weights(gpt, gpt_hf):
    ...
    for b in range(BASE_CONFIG["n_layers"]):
        ...
        gpt.transformer_blocks[b].attn.W_query.weight = assign_check(gpt.transformer_blocks[b].attn.W_query.weight, q_w.T)
        gpt.transformer_blocks[b].attn.W_key.weight = assign_check(gpt.transformer_blocks[b].attn.W_key.weight, k_w.T)
        gpt.transformer_blocks[b].attn.W_value.weight = assign_check(gpt.transformer_blocks[b].attn.W_value.weight, v_w.T)
    
        q_b, k_b, v_b = np.split(d[f"h.{b}.attn.c_attn.bias"], 3, axis=-1)
        gpt.transformer_blocks[b].attn.W_query.bias = assign_check(gpt.transformer_blocks[b].attn.W_query.bias, q_b)
        gpt.transformer_blocks[b].attn.W_key.bias = assign_check(gpt.transformer_blocks[b].attn.W_key.bias, k_b)
        gpt.transformer_blocks[b].attn.W_value.bias = assign_check(gpt.transformer_blocks[b].attn.W_value.bias, v_b)
    
    
        gpt.transformer_blocks[b].attn.out_proj.weight = assign_check(gpt.transformer_blocks[b].attn.out_proj.weight, d[f"h.{b}.attn.c_proj.weight"].T)
        gpt.transformer_blocks[b].attn.out_proj.bias = assign_check(gpt.transformer_blocks[b].attn.out_proj.bias, d[f"h.{b}.attn.c_proj.bias"])
